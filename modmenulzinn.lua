...
local function GetCustomTeamColor(p)
    ...
    if head then for _, v in pairs(head:GetDescendants()) do if v:IsA("TextLabel") and v.Text ~= "" then return v.TextColor3 end end end
    for _, v in pairs(char:GetDescendants()) do if v:IsA("TextLabel") and (string.find(string.lower(v.Text), string.lower(p.Name)) or string.find(string.lower(v.Text), string.lower(p.DisplayName))) then return v.TextColor3 end end
...
local function RegisterSearchable(element, text) table.insert(SearchableElements, {Element = element, OriginalParent = element.Parent, Text = string.lower(text)}) end
...
local function CreatePage(name)
    ...
    btn.MouseButton1Click:Connect(function() if SearchBar.Text ~= "" then SearchBar.Text = "" end; for _,v in pairs(Pages:GetChildren()) do v.Visible = false end; page.Visible = true; ActivePage = page end)
    return page, btn
end
...
local function CreateToggle(parent, text, callback, default)
    ...
    VisualToggles[text] = function(v, noCb)
        state = v; b.Text = text..": "..(state and "ON" or "OFF")
        b.BackgroundColor3 = state and Color3.fromRGB(50,100,50) or Color3.fromRGB(30,30,30); if not noCb then callback(state) end
    end
...
SearchBar:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    if query == "" then
        SearchResultsPage.Visible = false; TabsFrame.Visible = true; for _, data in pairs(SearchableElements) do data.Element.Parent = data.OriginalParent end; if ActivePage then ActivePage.Visible = true end
    else
        TabsFrame.Visible = false; for _, page in pairs(Pages:GetChildren()) do if page ~= SearchResultsPage then page.Visible = false end end; SearchResultsPage.Visible = true
...
local ModeBtn = Instance.new("TextButton", MiraPage); ModeBtn.Size = UDim2.new(1,-25,0,32); ModeBtn.BackgroundColor3 = Color3.fromRGB(30,30,30); ModeBtn.TextColor3 = Color3.new(1,1,1); ModeBtn.Text = "Prioridade: " .. Settings.PriorityMode; Instance.new("UICorner", ModeBtn); RegisterSearchable(ModeBtn, "Mudar Modo de Prioridade")
ModeBtn.MouseButton1Click:Connect(function() local idx = table.find(Modes, Settings.PriorityMode) or 1; idx = idx + 1; if idx > #Modes then idx = 1 end; Settings.PriorityMode = Modes[idx]; ModeBtn.Text = "Prioridade: " .. Settings.PriorityMode end)
...
local function UpdWhitelistList()
    ...
    for _,p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            local b = Instance.new("TextButton", WLScr); b.Size = UDim2.new(1,0,0,25); local isWl = Settings.Whitelist[p.UserId]; b.Text = p.DisplayName .. (isWl and " [SALVO]" or ""); b.BackgroundColor3 = isWl and Color3.fromRGB(0, 100, 50) or Color3.fromRGB(35,35,35); b.TextColor3 = Color3.new(1,1,1); b.TextSize = 10; b.Font = Enum.Font.GothamBold
            b.MouseButton1Click:Connect(function() Settings.Whitelist[p.UserId] = not Settings.Whitelist[p.UserId]; UpdWhitelistList() end)
        end
    end
...
local SecTrigger = CreateSection(TestePage, "TRIGGERBOT AUTO-HOLD")
...
local function GetKeyName(mod, key)
    local modStr = ""
    if mod == Enum.KeyCode.LeftAlt or mod == Enum.KeyCode.RightAlt then modStr = "Alt + "
    elseif mod == Enum.KeyCode.LeftControl or mod == Enum.KeyCode.RightControl then modStr = "Ctrl + "
    elseif mod == Enum.KeyCode.LeftShift or mod == Enum.KeyCode.RightShift then modStr = "Shift + " end
    return modStr .. (key.Name or "None")
end
...
local function UpdateToggleText(btn, baseText, bindKey)
    ...
    if bind then
        ...
        elseif bindKey == "Visuals" then stateStr = Settings.ESP and "ON" or "OFF"
        elseif bindKey == "Hitbox" then stateStr = Settings.HitboxEnabled and "ON" or "OFF" end
        btn.Text = textLimpo .. " [" .. GetKeyName(bind.Mod, bind.Key) .. "]: " .. stateStr
    end
end

local function CreateBindConfig(parent, labelText, bindKey, targetToggleBtn)
    local frame = Instance.new("Frame", parent); frame.Size = UDim2.new(1,-25,0,35); frame.BackgroundColor3 = Color3.fromRGB(30,30,30); Instance.new("UICorner", frame)
    local label = Instance.new("TextLabel", frame); label.Size = UDim2.new(0.6, 0, 1, 0); label.Position = UDim2.new(0,10,0,0); label.BackgroundTransparency = 1; label.TextColor3 = Color3.new(1,1,1); label.TextSize = 11; label.TextXAlignment = Enum.TextXAlignment.Left; label.Font = Enum.Font.GothamBold; label.Text = labelText
    local btn = Instance.new("TextButton", frame); btn.Size = UDim2.new(0.4, -10, 0.8, 0); btn.Position = UDim2.new(0.6, 0, 0.1, 0); btn.BackgroundColor3 = Color3.fromRGB(50,50,50); btn.TextColor3 = Color3.new(0,1,0); btn.TextSize = 10; btn.Font = Enum.Font.GothamBold; Instance.new("UICorner", btn)
    
    local bind = Settings.Binds[bindKey]
    btn.Text = GetKeyName(bind.Mod, bind.Key)
    
    btn.MouseButton1Click:Connect(function()
        btn.Text = "Pressione..."
        btn.TextColor3 = Color3.new(1, 0.8, 0)
        listeningBind = {Key = bindKey, UI = btn, Label = labelText, ToggleBtn = targetToggleBtn}
    end)
end

local SecBinds = CreateSection(BindsPage, "CONFIGURAR TECLAS")
CreateBindConfig(SecBinds, "Aimbot", "AimAssist", aimbotBtn)
...
UIS.InputBegan:Connect(function(input, gameProcessed)
    ...
    for bindKey, bindInfo in pairs(Settings.Binds) do
        if input.KeyCode == bindInfo.Key then
            local modMatch = true
            if bindInfo.Mod == Enum.KeyCode.LeftAlt and not (UIS:IsKeyDown(Enum.KeyCode.LeftAlt) or UIS:IsKeyDown(Enum.KeyCode.RightAlt)) then modMatch = false end
            if bindInfo.Mod == Enum.KeyCode.LeftControl and not (UIS:IsKeyDown(Enum.KeyCode.LeftControl) or UIS:IsKeyDown(Enum.KeyCode.RightControl)) then modMatch = false end
            if bindInfo.Mod == Enum.KeyCode.LeftShift and not (UIS:IsKeyDown(Enum.KeyCode.LeftShift) or UIS:IsKeyDown(Enum.KeyCode.RightShift)) then modMatch = false end
            if bindInfo.Mod == nil and (UIS:IsKeyDown(Enum.KeyCode.LeftAlt) or UIS:IsKeyDown(Enum.KeyCode.LeftControl) or UIS:IsKeyDown(Enum.KeyCode.LeftShift)) then modMatch = false end
            
            if modMatch then
                if bindKey == "AimAssist" and VisualToggles["Auxílio de Mira [Alt+2]"] then
                    local ns = not Settings.AimAssist
                    VisualToggles["Auxílio de Mira [Alt+2]"](ns, true); Settings.AimAssist = ns
                    UpdateToggleText(aimbotBtn, "Auxílio de Mira", "AimAssist")
                    SendNotification("[ATALHO] AIMBOT: " .. (ns and "ON" or "OFF"), ns)
                elseif bindKey == "Visuals" and VisualToggles["ESP Geral (Players) [Alt+3]"] then
                    local ns = not Settings.ESP
                    VisualToggles["ESP Geral (Players) [Alt+3]"](ns, true); Settings.ESP = ns
                    if VisualToggles["Chams"] then VisualToggles["Chams"](ns, true); Settings.Highlight = ns end
                    if VisualToggles["Team Color"] then VisualToggles["Team Color"](ns, true); Settings.TeamColor = ns end
                    UpdateToggleText(espBtn, "ESP Geral (Players)", "Visuals")
                    SendNotification("[ATALHO] VISUAIS: " .. (ns and "ON" or "OFF"), ns)
                elseif bindKey == "Hitbox" and VisualToggles["Hitbox Players [Alt+4]"] then
                    local ns = not Settings.HitboxEnabled
                    VisualToggles["Hitbox Players [Alt+4]"](ns, true); Settings.HitboxEnabled = ns
                    UpdateToggleText(hitboxBtn, "Hitbox Players", "Hitbox")
                    SendNotification("[ATALHO] HITBOX: " .. (ns and "ON" or "OFF"), ns)
                end
            end
        end
    end
...
local function CreateSkeletonLines() local lines = {} for i=1, 12 do local l = Drawing.new("Line"); l.Thickness = 1; l.Visible = false; l.Color = Color3.new(1,1,1); table.insert(lines, l) end return lines end
local function RemoveESP(container, obj) if container[obj] then if container[obj].Box then container[obj].Box:Remove() end; if container[obj].Name then container[obj].Name:Remove() end; if container[obj].Dist then container[obj].Dist:Remove() end; if container[obj].Line then container[obj].Line:Remove() end; if container[obj].Highlight then container[obj].Highlight:Destroy() end; if container[obj].Skeleton then for _, l in pairs(container[obj].Skeleton) do l:Remove() end end; container[obj] = nil end end
local function CreateESPObj(container, obj) if container[obj] then return end; container[obj] = { Box = Drawing.new("Square"), Name = Drawing.new("Text"), Dist = Drawing.new("Text"), Line = Drawing.new("Line"), Highlight = nil, Skeleton = CreateSkeletonLines() }; local e = container[obj]; e.Box.Thickness = 1.5; e.Box.Filled = false; e.Name.Size = 14; e.Name.Center = true; e.Name.Outline = true; e.Dist.Size = 12; e.Dist.Center = true; e.Dist.Outline = true; e.Line.Thickness = 1 end
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.AimAssist then
        ...
        local function CheckTarget(part, humanoid)
            if humanoid and humanoid.Health > 1 and IsVisible(part) then
                local predPos = part.Position
                if Settings.AimPrediction then predPos = predPos + (part.AssemblyLinearVelocity * Settings.PredictionVelocity) end
                ...
                if Settings.TargetPriority then
                    ...
                    if vis then
                        ...
                        if mag < Settings.AimFOV and mag < bestValue then bestValue = mag; target = part; targetPredPos = predPos end
                    end
                end
            end
        end
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(Settings.AimPart) then
                ...
                if Settings.ColorAimbot and Settings.ColorAimbotTarget then
                    ...
                    local diff = math.abs(Settings.ColorAimbotTarget.R - targetColor.R) + math.abs(Settings.ColorAimbotTarget.G - targetColor.G) + math.abs(Settings.ColorAimbotTarget.B - targetColor.B)
                    if diff > 0.2 then continue end
                elseif Settings.AutoTeamColorCheck then
                    local myColor, targetColor = GetCustomTeamColor(LocalPlayer), GetCustomTeamColor(p)
                    local diff = math.abs(myColor.R - targetColor.R) + math.abs(myColor.G - targetColor.G) + math.abs(myColor.B - targetColor.B)
                    if diff < 0.2 then continue end
                elseif Settings.TeamCheck and p.Team == LocalPlayer.Team then 
                    continue 
                end
                CheckTarget(p.Character[Settings.AimPart], p.Character:FindFirstChild("Humanoid"))
            end
        end
    ...
    local function RenderESP(container, isNPC)
        for obj, e in pairs(container) do
            ...
            if not character or not character:FindFirstChild("HumanoidRootPart") or (isNPC and not Settings.ESPNPC) then
                e.Box.Visible = false; e.Name.Visible = false; e.Dist.Visible = false; e.Line.Visible = false; if e.Highlight then e.Highlight.Enabled = false end; continue
            end
            ...
            if isNPC then color = Color3.fromRGB(255, 80, 80)
            elseif Settings.AutoTeamColorCheck then color = GetCustomTeamColor(obj)
            elseif Settings.TeamColor and obj.TeamColor then color = obj.TeamColor.Color end
            ...
            if (isNPC and vis) or (not isNPC and Settings.ESP and vis) then
                ...
                if Settings.Highlight then if not e.Highlight or e.Highlight.Parent ~= character then if e.Highlight then e.Highlight:Destroy() end e.Highlight = Instance.new("Highlight", character) end; e.Highlight.Enabled = true; e.Highlight.FillColor = color; e.Highlight.FillTransparency = 0.5 elseif e.Highlight then e.Highlight.Enabled = false end
            else 
