...
local VERSION = "v4.9.2"
local CHANGELOG_TEXT = [[
--- CORREÇÕES v4.9.2 ---
[!] BUG FIX: ESP Names/Boxes agora somem instantaneamente quando o player sai.
[+] ESTABILIDADE: Refatoração da função de limpeza de memória (Drawing Library).
[+] LOGICA: Preparação de ambiente para a próxima aba (MIRA).
-------------------------]]
...
local function CreatePage(name)
    ...
    local page = Instance.new("ScrollingFrame", Pages); page.Size = UDim2.new(1,0,1,0); page.BackgroundTransparency = 1; page.Visible = false; page.ScrollBarThickness = 3; page.ScrollBarImageColor3 = Color3.new(1,1,1)
...
CreateToggle(ESPPage, "Names", function(v) Settings.Names = v end, false)
CreateToggle(ESPPage, "Distance", function(v) Settings.Distance = v end, false)
...
-- TP
local SectionSelect = CreateSection(TPPage, "SELEÇÃO DE JOGADOR")
local SectionAction = CreateSection(TPPage, "AÇÕES DE TELEPORTE")
...
local pListScroll = Instance.new("ScrollingFrame", PlayerListFrame); pListScroll.Size = UDim2.new(1,0,1,0); pListScroll.BackgroundTransparency = 1; pListScroll.ScrollBarThickness = 2
Instance.new("UIListLayout", pListScroll).Padding = UDim.new(0,2)
...
local function UpdateList()
    ...
    for _,p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer then
        local b = Instance.new("TextButton", pListScroll); b.Size = UDim2.new(1,0,0,25); b.Text = p.DisplayName; b.BackgroundColor3 = Color3.fromRGB(35,35,35); b.TextColor3 = Color3.new(1,1,1)
        b.MouseButton1Click:Connect(function() Settings.SelectedPlayer = p; SelectedLabel.Text = "Alvo: "..p.DisplayName end)
    end end
    pListScroll.CanvasSize = UDim2.new(0,0,0,pListScroll.UIListLayout.AbsoluteContentSize.Y)
end
...
CreateStepper(SectionAction, "Distância", 1, 10, 3, 1, function(v) Settings.StickyDistance = v end)
...
CreateToggle(FPSPage, "Sem Sombra", function(v) Lighting.GlobalShadows = not v end)
...
-- LÓGICA DE LIMPEZA E RENDER
local ESPContainer = {}

local function RemoveESP(p)
    if ESPContainer[p] then
        -- Remove todos os desenhos da Drawing Library
...
local function CreateESP(p)
    ...
    RemoveESP(p) -- Garante que não haja duplicatas
    ESPContainer[p] = {
...
Players.PlayerRemoving:Connect(RemoveESP) -- GATILHO DE CORREÇÃO DO BUG
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.AutoNearest then
        ...
        if nearest then Settings.SelectedPlayer = nearest; SelectedLabel.Text = "Alvo: "..nearest.DisplayName end
    end
...
ToggleBtn.MouseButton1Up:Connect(function() 
    ...
    if Main.Visible then 
        MenuAberto = false; local tw = TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In); local anim = TweenService:Create(Main, tw, {Size = UDim2.new(0, 300, 0, 0), BackgroundTransparency = 1}); TweenService:Create(stroke, tw, {Transparency = 1}):Play(); TweenService:Create(Title, tw, {TextTransparency = 1}):Play(); anim:Play(); anim.Completed:Connect(function() Main.Visible = false end)
    else 
        MenuAberto = true; Main.Visible = true; Main.Position = UDim2.new(0.5, -150, 0.5, -175); local tw = TweenInfo.new(0.5, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out); TweenService:Create(Main, tw, {Size = UDim2.new(0, 300, 0, 350), BackgroundTransparency = 0.1}):Play(); TweenService:Create(stroke, tw, {Transparency = 0}):Play(); TweenService:Create(Title, tw, {TextTransparency = 0}):Play(); 
    end
end)
