...
getgenv().Settings = {
    ...
    AimAssist = false, AimPart = "Head", AimFOV = 100, AimSmooth = 0.1, ShowFOV = false, WallCheck = false, TeamCheck = false, AimNPC = false
}

local VERSION = "v5.0.9"
local CHANGELOG_TEXT = [[
--- RESTAURAÇÃO v5.0.9 ---
[!] FIX: Todas as abas e opções restauradas.
[+] MIRA: Readicionado seletor Cabeça/Tronco.
[+] MIRA: Suporte a NPCs e Team Check ativos.
[+] ESP: Anti-Ghosting agressivo ativado.
[+] UI: Tamanho otimizado e Reset de Posição.
-------------------------]]
...
Main.Size = UDim2.new(0,280,0,320); Main.Position = UDim2.new(0.5, -140, 0.5, -160); Main.BackgroundColor3 = Color3.fromRGB(10,10,10); Main.ClipsDescendants = true; Main.Visible = false; Main.BackgroundTransparency = 1
...
task.spawn(function()
    local chars = "ABCDEFGHIJKLMN1234567890!@#$"
    while true do
        local color = Color3.fromHSV(tick() % 5 / 5, 1, 1)
        Title.TextColor3 = color; stroke.Color = color
        if MenuAberto then
            if math.random(1, 20) == 1 then
                local rand = ""; for i=1,15 do rand ..= chars:sub(math.random(1,#chars),math.random(1,#chars)) end
                Title.Text = rand; task.wait(0.05)
            end
            Title.Text = "Kiko MENU | " .. VERSION
        end
        task.wait(0.05)
    end
end)

local FPSLabel = Instance.new("TextLabel", Main); FPSLabel.Size = UDim2.new(0,50,0,35); FPSLabel.Position = UDim2.new(1,-60,0,0); FPSLabel.Text = "FPS: 0"; FPSLabel.BackgroundTransparency = 1; FPSLabel.TextColor3 = Color3.new(0,1,0); FPSLabel.TextSize = 11; FPSLabel.Font = Enum.Font.Code

-- ABAS
local function CreatePage(name)
    local Pages = Main:FindFirstChild("Pages") or Instance.new("Frame", Main); Pages.Name = "Pages"; Pages.Position = UDim2.new(0,0,0,70); Pages.Size = UDim2.new(1,0,1,-70); Pages.BackgroundTransparency = 1
    local Tabs = Main:FindFirstChild("Tabs") or Instance.new("ScrollingFrame", Main); Tabs.Name = "Tabs"; Tabs.Size = UDim2.new(1,0,0,35); Tabs.Position = UDim2.new(0,0,0,35); Tabs.BackgroundTransparency = 1; Tabs.ScrollBarThickness = 0; Tabs.CanvasSize = UDim2.new(0,0,0,0)
    local Layout = Tabs:FindFirstChild("Layout") or Instance.new("UIListLayout", Tabs); Layout.Name = "Layout"; Layout.FillDirection = Enum.FillDirection.Horizontal
    
    local btn = Instance.new("TextButton", Tabs); btn.Size = UDim2.new(0, 70, 1, 0); btn.Text = name; btn.BackgroundTransparency = 1; btn.TextColor3 = Color3.new(1,1,1); btn.TextSize = 10; btn.Font = Enum.Font.GothamBold
    ...
    Instance.new("UIListLayout", page).Padding = UDim.new(0,5); page.UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    
    btn.MouseButton1Click:Connect(function() for _,v in pairs(Pages:GetChildren()) do v.Visible = false end; page.Visible = true end)
    RunService.RenderStepped:Connect(function() page.CanvasSize = UDim2.new(0,0,0,page.UIListLayout.AbsoluteContentSize.Y + 10) end)
    return page
end

local function CreateSection(parent, name)
    local btn = Instance.new("TextButton", parent); btn.Size = UDim2.new(1,-20,0,28); btn.Text = "[ " .. name .. " ]"; btn.BackgroundColor3 = Color3.fromRGB(25,25,25); btn.TextColor3 = Color3.fromRGB(200,200,200); btn.Font = Enum.Font.GothamBold; btn.TextSize = 11; Instance.new("UICorner", btn)
    local cont = Instance.new("Frame", parent); cont.Size = UDim2.new(1,0,0,0); cont.AutomaticSize = Enum.AutomaticSize.Y; cont.BackgroundTransparency = 1; cont.Visible = false
    Instance.new("UIListLayout", cont).Padding = UDim.new(0,5); cont.UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
    btn.MouseButton1Click:Connect(function() cont.Visible = not cont.Visible; btn.BackgroundColor3 = cont.Visible and Color3.fromRGB(40,40,40) or Color3.fromRGB(25,25,25) end)
    return cont
end
...
local function CreateToggle(parent, text, callback, default)
    ...
    b.MouseButton1Click:Connect(function() local s = b.Text:find("OFF"); b.Text = text..": "..(s and "ON" or "OFF"); b.BackgroundColor3 = s and Color3.fromRGB(50,100,50) or Color3.fromRGB(30,30,30); callback(s ~= nil) end)
end
...
local function CreateStepper(parent, text, min, max, default, step, callback)
    ...
    local m = Instance.new("TextButton", frame); m.Size = UDim2.new(0,35,0,25); m.Position = UDim2.new(0,40,0,20); m.Text = "-"; m.BackgroundColor3 = Color3.fromRGB(40,40,40); m.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", m)
    local p = Instance.new("TextButton", frame); p.Size = UDim2.new(0,35,0,25); p.Position = UDim2.new(0,140,0,20); p.Text = "+"; p.BackgroundColor3 = Color3.fromRGB(40,40,40); p.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", p)
    local val = default; local function up(n) val = math.clamp(n, min, max); label.Text = text..": "..string.format("%.2f", val); callback(val) end
    m.MouseButton1Click:Connect(function() up(val - step) end); p.MouseButton1Click:Connect(function() up(val + step) end)
end
...
-- LÓGICA ESP & CLEANUP
local ESPCont = {}
local function RemoveESP(p) if ESPCont[p] then for _,v in pairs(ESPCont[p]) do if typeof(v) == "table" and v.Remove then v:Remove() end end; if ESPCont[p].Highlight then ESPCont[p].Highlight:Destroy() end; ESPCont[p] = nil end end
local function CreateESP(p) if p == LocalPlayer then return end; RemoveESP(p); ESPCont[p] = {Box = Drawing.new("Square"), Name = Drawing.new("Text"), Line = Drawing.new("Line"), Dist = Drawing.new("Text")}; local e = ESPCont[p]; e.Box.Thickness = 1.5; e.Box.Filled = false; e.Name.Size = 14; e.Name.Center = true; e.Name.Outline = true; e.Line.Thickness = 1 end
Players.PlayerRemoving:Connect(RemoveESP); Players.PlayerAdded:Connect(CreateESP); for _, p in pairs(Players:GetPlayers()) do CreateESP(p) end

-- RENDER LOOP
RunService.RenderStepped:Connect(function()
    FPSLabel.Text = "FPS: " .. math.floor(1/RunService.RenderStepped:Wait())
    FOVCircle.Visible = Settings.ShowFOV; FOVCircle.Radius = Settings.AimFOV; FOVCircle.Position = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2); FOVCircle.Color = stroke.Color; FOVCircle.Thickness = 1
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(Settings.AimPart) then
                ...
                if vis then
                    ...
                    if mag < d then d, t = mag, p.Character[Settings.AimPart] end
                end
            end
        end
        if Settings.AimNPC then -- NPC CHECK
             for _, o in pairs(workspace:GetDescendants()) do
                if o:IsA("Model") and o:FindFirstChild("Humanoid") and not Players:GetPlayerFromCharacter(o) then
                    local p = o:FindFirstChild(Settings.AimPart) or o:FindFirstChild("HumanoidRootPart")
                    if p then
                        local pos, vis = Camera:WorldToViewportPoint(p.Position)
                        if vis then
                            local mag = (Vector2.new(pos.X, pos.Y) - Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)).Magnitude
                            if mag < d then d, t = mag, p end
                        end
                    end
                end
            end
        end
        if t then Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, t.Position), Settings.AimSmooth) end
    end

    -- ESP DRAW
    for p, e in pairs(ESPCont) do
        if not p.Parent then RemoveESP(p) continue end
        if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and Settings.ESP then
            ...
            if vis then
                ...
                if Settings.Highlight then if not e.Highlight or e.Highlight.Parent ~= p.Character then e.Highlight = Instance.new("Highlight", p.Character) end e.Highlight.Enabled = true; e.Highlight.FillColor = color else if e.Highlight then e.Highlight.Enabled = false end end
            else e.Box.Visible = false; e.Name.Visible = false; e.Line.Visible = false; if e.Highlight then e.Highlight.Enabled = false end end
        else e.Box.Visible = false; e.Name.Visible = false; e.Line.Visible = false end
    end
end)

ToggleBtn.MouseButton1Up:Connect(function() 
    if Main.Visible then MenuAberto = false; Main.Visible = false
    else MenuAberto = true; Main.Visible = true; Main.Position = UDim2.new(0.5, -140, 0.5, -160) end
end)
local function MakeDraggable(gui, isMenu)
    local dragging, dragInput, dragStart, startPos
    gui.InputBegan:Connect(function(input)
...
local function CreatePage(name)
    local Pages = Main:FindFirstChild("Pages") or Instance.new("Frame", Main); Pages.Name = "Pages"; Pages.Position = UDim2.new(0,0,0,70); Pages.Size = UDim2.new(1,0,1,-70); Pages.BackgroundTransparency = 1
    local TabsFrame = Main:FindFirstChild("Tabs") or Instance.new("ScrollingFrame", Main); TabsFrame.Name = "Tabs"; TabsFrame.Size = UDim2.new(1,0,0,35); TabsFrame.Position = UDim2.new(0,0,0,35); TabsFrame.BackgroundTransparency = 1; TabsFrame.ScrollBarThickness = 0; TabsFrame.CanvasSize = UDim2.new(0,0,0,0)
    local TabList = TabsFrame:FindFirstChild("Layout") or Instance.new("UIListLayout", TabsFrame); TabList.Name = "Layout"; TabList.FillDirection = Enum.FillDirection.Horizontal
...
-- [ABAS TP/HITBOX/FPS/INFOS MANTIDAS]
CreateToggle(FPSPage, "Remover Sombras", function(v) Lighting.GlobalShadows = not v end)
...
-- LÓGICA ANTI-GHOST (CORE)
local ESPContainer = {}
local function ClearAllDrawings()
    for p, drawings in pairs(ESPContainer) do
        for _, obj in pairs(drawings) do if typeof(obj) == "table" and obj.Remove then obj:Remove() end end
...
local function RemoveESP(p)
    if ESPContainer[p] then
        for _, obj in pairs(ESPContainer[p]) do if typeof(obj) == "table" and obj.Remove then obj:Remove() end end
...
local function CreateESP(p)
    ...
    e.Box.Thickness = 1.5; e.Box.Filled = false; e.Name.Size = 14; e.Name.Center = true; e.Name.Outline = true; e.Line.Thickness = 1
end
...
local function GetAimbotTarget()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(Settings.AimPart) then
            ...
            if vis then
                local mag = (Vector2.new(pos.X, pos.Y) - Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)).Magnitude
                if mag < minDist then minDist, target = mag, p.Character[Settings.AimPart] end
            end
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.AimAssist then
        ...
        if targetPart then Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, targetPart.Position), Settings.AimSmooth) end
    end

    -- ESP Render (com verificação de existência do player)
    for p, e in pairs(ESPContainer) do
        if not p or not p.Parent then RemoveESP(p) continue end -- Limpeza secundária
        if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and Settings.ESP then
            local hrp = p.Character.HumanoidRootPart; local pos, vis = Camera:WorldToViewportPoint(hrp.Position)
            local color = (Settings.TeamColor and p.TeamColor) and p.TeamColor.Color or Color3.new(1,1,1)
            if vis then
...
ToggleBtn.MouseButton1Up:Connect(function() 
    if Main.Visible then 
        ...
        MenuAberto = true; Main.Visible = true; Main.Position = UDim2.new(0.5, -140, 0.5, -160)
