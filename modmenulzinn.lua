...
local VERSION = "v5.0.5"
local CHANGELOG_TEXT = [[
--- HISTÓRICO v5.0.5 ---
[+] MIRA: Adicionado 'Team Check' (ignora amigos automaticamente).
[!] FIX: Título 'Kiko MENU' forçado visível com efeito Hacker.
[+] VISUAL: FOV apenas com borda fina (RGB Sync).
[+] ESTABILIDADE: Melhoria na detecção de times em jogos diversos.
-------------------------]]
...
Title.Size = UDim2.new(1,0,0,35); Title.BackgroundTransparency = 1; Title.TextSize = 18; Title.Font = Enum.Font.Code; Title.TextTransparency = 0
...
local FPSLabel = Instance.new("TextLabel", Main); FPSLabel.Size = UDim2.new(0,50,0,35); FPSLabel.Position = UDim2.new(1,-60,0,0); FPSLabel.Text = "FPS: 0"; FPSLabel.BackgroundTransparency = 1; FPSLabel.TextColor3 = Color3.new(0,1,0); FPSLabel.TextSize = 12; FPSLabel.Font = Enum.Font.Code; FPSLabel.TextTransparency = 1
...
CreateToggle(MiraPage, "Team Check", function(v) Settings.TeamCheck = v end) -- Nova Função
...
local SelLab = Instance.new("TextLabel", SecSel); SelLab.Size = UDim2.new(1,-20,0,30); SelLab.Text = "Alvo: Nenhum"; SelLab.TextColor3 = Color3.new(0,1,0); SelLab.BackgroundTransparency = 1
...
local function UpList() for _,v in pairs(PLScr:GetChildren()) do if v:IsA("TextButton") then v:Destroy() end end; for _,p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer then local b = Instance.new("TextButton", PLScr); b.Size = UDim2.new(1,0,0,25); b.Text = p.DisplayName; b.BackgroundColor3 = Color3.fromRGB(35,35,35); b.TextColor3 = Color3.new(1,1,1); b.MouseButton1Click:Connect(function() Settings.SelectedPlayer = p; SelLab.Text = "Alvo: "..p.DisplayName end) end end; PLScr.CanvasSize = UDim2.new(0,0,0,PLScr.UIListLayout.AbsoluteContentSize.Y) end
...
local function IsVisible(part)
    ...
    params.FilterType = Enum.RaycastFilterType.Exclude; params.FilterDescendantsInstances = ignoreList
