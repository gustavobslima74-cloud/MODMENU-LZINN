...
v4.1: Base v3.8 Restaurada. UI com ajuste de altura automático por aba.
v3.8: ESP Team Color (Fiel ao original).
...
--// FUNÇÃO DRAG (APENAS PARA O MENU)
...
Main.Size = UDim2.new(0,300,0,100) -- Inicia pequeno
...
TabsFrame.CanvasSize = UDim2.new(1.5,0,0,0) -- Scroll horizontal
...
-- FUNÇÃO AJUSTE DE TAMANHO AUTOMÁTICO
...
local function CreatePage(name)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        page.Visible = true
        UpdateMenuSize(layout.AbsoluteContentSize.Y)
    end)
    
    layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        page.CanvasSize = UDim2.new(0,0,0,layout.AbsoluteContentSize.Y + 10)
        if page.Visible then UpdateMenuSize(layout.AbsoluteContentSize.Y) end
    end)
        ...
        if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and Settings.ESP then
            ...
            if vis then
                if Settings.Boxes then
                    e.Box.Visible = true; e.Box.Size = Vector2.new(2500/pos.Z, 3500/pos.Z)
                    e.Box.Position = Vector2.new(pos.X - e.Box.Size.X/2, pos.Y - e.Box.Size.Y/2); e.Box.Color = color
                else e.Box.Visible = false end
                
                e.Name.Visible = Settings.Names; e.Name.Text = p.DisplayName; e.Name.Position = Vector2.new(pos.X, pos.Y - (2000/pos.Z) - 20); e.Name.Color = color
                e.Dist.Visible = Settings.Distance; e.Dist.Text = math.floor((hrp.Position - Camera.CFrame.Position).Magnitude).."m"; e.Dist.Position = Vector2.new(pos.X, pos.Y + (2000/pos.Z) + 5); e.Dist.Color = Color3.new(0,1,0)
                
                if Settings.Lines and head then
                    local headPos = Camera:WorldToViewportPoint(head.Position)
                    e.Line.Visible = true; e.Line.From = Vector2.new(Camera.ViewportSize.X/2, 0); e.Line.To = Vector2.new(headPos.X, headPos.Y); e.Line.Color = color
