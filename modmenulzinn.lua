...
v4.8: RESTAURAÇÃO TOTAL. Todas as abas (FPS, Hitbox, Player, ESP) voltaram.
v4.7: Abas Teleporte e Mira.
v4.6: Third Person Force.
----------------------------]]
...
Main.Size = UDim2.new(0,300,0,400); Main.Position = UDim2.new(0.5, -150, 0.5, -200); Main.BackgroundColor3 = Color3.fromRGB(10,10,10); Main.ClipsDescendants = true; Main.Visible = false; Main.BackgroundTransparency = 1
...
local function UpdateMenuSize(pageSizeY)
    local targetHeight = math.clamp(pageSizeY + 120, 250, 500)
    TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, 300, 0, targetHeight)}):Play()
end

local function CreatePage(name)
    local btn = Instance.new("TextButton", TabsFrame); btn.Size = UDim2.new(0, 75, 1, 0); btn.Text = name; btn.BackgroundTransparency = 1; btn.TextColor3 = Color3.new(1,1,1); btn.TextTransparency = 1; btn.TextSize = 11
...
-- COMPONENTES PADRÃO (TOGGLE E STEPPER)
...
CreateToggle(ESPPage, "Nomes", function(v) Settings.Names = v end)
CreateToggle(ESPPage, "Distancia", function(v) Settings.Distance = v end)
CreateToggle(ESPPage, "Lines", function(v) Settings.Lines = v end)
CreateToggle(ESPPage, "Chams", function(v) Settings.Highlight = v end)

-- ABA PLAYER (RESTAURADA)
...
CreateStepper(PlayerPage, "Pulo", 50, 500, 50, 5, function(v) Settings.JumpPower = v end)
CreateToggle(PlayerPage, "Pulo Infinito", function(v) Settings.InfiniteJump = v end)

-- ABA TELEPORTE (RESTAURADA)
local SelectedLabel = Instance.new("TextLabel", TeleportPage); SelectedLabel.Size = UDim2.new(1,-20,0,30); SelectedLabel.Text = "Selecionado: Nenhum"; SelectedLabel.TextColor3 = Color3.new(0,1,0); SelectedLabel.BackgroundTransparency = 1
local ListVisible = false
local PlayerListFrame = Instance.new("ScrollingFrame", TeleportPage); PlayerListFrame.Size = UDim2.new(1,-20,0,0); PlayerListFrame.BackgroundColor3 = Color3.fromRGB(20,20,20); PlayerListFrame.Visible = false
...
local function UpdateList()
    ...
    for _,p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer then
        local b = Instance.new("TextButton", PlayerListFrame); b.Size = UDim2.new(1,0,0,25); b.Text = p.DisplayName; b.BackgroundColor3 = Color3.fromRGB(35,35,35); b.TextColor3 = Color3.new(1,1,1)
        b.MouseButton1Click:Connect(function() Settings.SelectedPlayer = p; SelectedLabel.Text = "Selecionado: "..p.DisplayName end)
    end end
end

local ToggleListBtn = Instance.new("TextButton", TeleportPage); ToggleListBtn.Size = UDim2.new(1,-20,0,35); ToggleListBtn.Text = "Abrir Lista de Players"; ToggleListBtn.BackgroundColor3 = Color3.fromRGB(40,40,40); ToggleListBtn.TextColor3 = Color3.new(1,1,1)
ToggleListBtn.MouseButton1Click:Connect(function() 
    ListVisible = not ListVisible
    PlayerListFrame.Visible = ListVisible
    PlayerListFrame.Size = ListVisible and UDim2.new(1,-20,0,100) or UDim2.new(1,-20,0,0)
    ToggleListBtn.Text = ListVisible and "Fechar Lista" or "Abrir Lista de Players"
    UpdateList()
end)

local TpBtn = Instance.new("TextButton", TeleportPage); TpBtn.Size = UDim2.new(1,-20,0,40); TpBtn.Text = "TELEPORTAR"; TpBtn.BackgroundColor3 = Color3.fromRGB(0,100,200); TpBtn.TextColor3 = Color3.new(1,1,1)
...
-- ABA MIRA
local MiraText = Instance.new("TextLabel", MiraPage); MiraText.Size = UDim2.new(1,0,0,50); MiraText.Text = "EM BREVE..."; MiraText.TextColor3 = Color3.new(1,1,1); MiraText.BackgroundTransparency = 1

-- ABA HITBOX (RESTAURADA)
...
CreateToggle(FPSPage, "Otimizar Texturas", function(v) 
    Settings.BoostFPS = v
    for _,o in pairs(game:GetDescendants()) do if o:IsA("Texture") or o:IsA("Decal") then o.Transparency = v and 1 or 0 end end
end)
...
-- LÓGICA RENDER (FPS / ESP / STICKY)
local lastUpdate = tick(); local frameCount = 0
RunService.RenderStepped:Connect(function()
    frameCount += 1; if tick() - lastUpdate >= 1 then FPSLabel.Text = "FPS: "..frameCount; frameCount = 0; lastUpdate = tick() end
    ...
    if Settings.UseJump and LocalPlayer.Character then LocalPlayer.Character.Humanoid.JumpPower = Settings.JumpPower end
    if Settings.StickyBehind and Settings.SelectedPlayer and Settings.SelectedPlayer.Character then
        LocalPlayer.Character.HumanoidRootPart.CFrame = Settings.SelectedPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,3)
    end
end)
...
local function OpenUI()
    MenuAberto = true; Main.Visible = true; Main.Position = UDim2.new(0.5, -150, 0.5, -200)
    local tw = TweenInfo.new(0.5, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out)
    TweenService:Create(Main, tw, {Size = UDim2.new(0, 300, 0, 400), BackgroundTransparency = 0.1}):Play()
    TweenService:Create(stroke, tw, {Transparency = 0}):Play(); TweenService:Create(Title, tw, {TextTransparency = 0}):Play(); TweenService:Create(FPSLabel, tw, {TextTransparency = 0}):Play()
